...
function runTests() {
  ...
  if (
    test('returns default structure when no file exists', () => {
      resetAliases();
  ...
  if (
    test('returns default structure for corrupted JSON', () => {
      const aliasesPath = aliases.getAliasesPath();
  ...
  if (
    test('returns default structure for invalid structure', () => {
      const aliasesPath = aliases.getAliasesPath();
  ...
  if (
    test('updates an existing alias', () => {
  ...
  if (
    test('rejects invalid characters in alias', () => {
  ...
  if (
    test('rejects alias longer than 128 chars', () => {
  ...
  if (
    test('rejects reserved alias names', () => {
  ...
  if (
    test('rejects empty session path', () => {
  ...
  if (
    test('accepts underscores and dashes in alias', () => {
      resetAliases();
  ...
  if (
    test('returns null for invalid alias characters', () => {
  ...
  if (
    test('lists all aliases sorted by recency', () => {
      resetAliases();
  ...
  if (
    test('filters aliases by search string', () => {
  ...
  if (
    test('returns empty array when no aliases exist', () => {
      resetAliases();
      const list = aliases.listAliases();
  ...
  if (
    test('returns error for non-existent alias', () => {
  ...
  if (
    test('rejects rename to existing alias', () => {
      resetAliases();
  ...
  if (
    test('rejects rename of non-existent alias', () => {
  ...
  if (
    test('rejects rename to invalid characters', () => {
      resetAliases();
  ...
  if (
    test('rejects rename to empty string', () => {
      resetAliases();
      aliases.setAlias('valid', '/path');
  ...
  if (
    test('rejects rename to reserved name', () => {
      resetAliases();
      aliases.setAlias('valid', '/path');
  ...
  if (
    test('rejects rename to name exceeding 128 chars', () => {
      resetAliases();
      aliases.setAlias('valid', '/path');
  ...
  if (
    test('updates title of existing alias', () => {
      resetAliases();
  ...
  if (
    test('rejects non-string non-null title', () => {
  ...
  if (
    test('rejects title update for non-existent alias', () => {
  ...
  if (
    test('resolves alias to session path', () => {
      resetAliases();
  ...
  if (
    test('returns input as-is when not an alias', () => {
  ...
  if (
    test('finds all aliases for a session path', () => {
      resetAliases();
  ...
  if (
    test('returns empty array for session with no aliases', () => {
  ...
  if (
    test('removes aliases for non-existent sessions', () => {
      resetAliases();
  ...
  if (
    test('handles all sessions existing (no cleanup needed)', () => {
      resetAliases();
  ...
  if (
    test('rejects non-function sessionExists', () => {
  ...
  if (
    test('handles sessionExists that throws an exception', () => {
      resetAliases();
  ...
  if (
    test('handles entries with missing timestamps gracefully', () => {
      resetAliases();
      const data = aliases.loadAliases();
  ...
  if (
    test('search matches title in addition to name', () => {
      resetAliases();
  ...
  if (
    test('rejects non-string session path types', () => {
      resetAliases();
  ...
  if (
    test('rejects whitespace-only session path', () => {
      resetAliases();
      const result = aliases.setAlias('valid-name', '   ');
      assert.strictEqual(result.success, false);
      assert.ok(result.error.includes('empty'));
  ...
  if (
    test('empty string title becomes null', () => {
      resetAliases();
  ...
  if (
    test('persists data across load/save cycles', () => {
      resetAliases();
      const data = aliases.loadAliases();
  ...
  if (
    test('returns correct totalChecked when all removed', () => {
      resetAliases();
  ...
  if (
    test('cleanupAliases returns success:true when aliases removed', () => {
      resetAliases();
  ...
  if (
    test('cleanupAliases returns success:true when no cleanup needed', () => {
      resetAliases();
      aliases.setAlias('alive', '/sessions/alive');
      const result = aliases.cleanupAliases(() => true);
      assert.strictEqual(result.success, true);
  ...
  if (
    test('cleanupAliases with empty aliases file does nothing', () => {
      resetAliases();
      const result = aliases.cleanupAliases(() => true);
      assert.strictEqual(result.success, true);
      assert.strictEqual(result.removed, 0);
  ...
  if (
    test('cleanupAliases preserves aliases where sessionExists returns true', () => {
      resetAliases();
  ...
  if (
    test('rename preserves session path and title', () => {
      resetAliases();
  ...
  if (
    test('rename preserves original createdAt timestamp', () => {
      resetAliases();
  ...
  if (
    test('does not match partial session paths', () => {
      resetAliases();
  ...
  if (
    test('rejects uppercase reserved name LIST', () => {
      resetAliases();
  ...
  if (
    test('rejects mixed-case reserved name Help', () => {
      resetAliases();
  ...
  if (
    test('rejects mixed-case reserved name Set', () => {
      resetAliases();
  ...
  if (
    test('negative limit does not truncate results', () => {
      resetAliases();
  ...
  if (
    test('undefined title becomes null (same as explicit null)', () => {
      resetAliases();
  ...
  if (
    test('saveAliases returns false for invalid data (non-serializable)', () => {
  ...
  if (
    test('saveAliases handles writing to read-only directory gracefully', () => {
  ...
  if (
    test('loadAliases returns fresh structure for missing file', () => {
      resetAliases();
      const data = aliases.loadAliases();
  ...
  if (
    test('renameAlias with circular data triggers rollback path', () => {
  ...
  if (
    test('renameAlias returns rolled-back error message on save failure', () => {
  ...
  if (
    test('renameAlias rollback preserves original alias data on naming conflict', () => {
      resetAliases();
  ...
  if (
    test('saveAliases creates backup before write and removes on success', () => {
      resetAliases();
  ...
  if (
    test('saveAliases with non-serializable data returns false and preserves existing file', () => {
      resetAliases();
      ...
      assert.ok(
        contentAfter.includes('before-fail'),
  ...
  if (
    test('saveAliases overwrites existing file atomically', () => {
  ...
  if (
    test('rapid sequential setAlias calls maintain data integrity', () => {
      resetAliases();
      for (let i = 0; i < 5; i++) {
  ...
  if (
    test('Windows platform mock: unlinks existing file before rename', () => {
      resetAliases();
      ...
      try {
        ...
        assert.strictEqual(
          data.aliases['win-updated'].sessionPath,
          '2026-02-01-def456-session.tmp',
          'Session path should match'
        );
  ...
  if (
    test('loadAliases backfills missing version and metadata fields', () => {
      resetAliases();
      const aliasesPath = aliases.getAliasesPath();
      ...
      assert.strictEqual(
        ...
        1,
        'Metadata totalCount should match alias count'
      );
  ...
  if (
    test('loadAliases returns default structure for empty (0-byte) file', () => {
      resetAliases();
      const aliasesPath = aliases.getAliasesPath();
  ...
  if (
    test('resolveSessionAlias returns null when given null input', () => {
      resetAliases();
  ...
  if (
    test('loadAliases backfills only metadata when version already present', () => {
      resetAliases();
      const aliasesPath = aliases.getAliasesPath();
  ...
  if (
    test('updateAliasTitle returns failure when saveAliases fails (read-only dir)', () => {
      ...
      try {
        ...
        assert.ok(
          result.error.includes('Failed to update alias title'),
          `Should return save failure error, got: ${result.error}`
        );
      } finally {
        try {
          fs.chmodSync(isoClaudeDir, 0o755);
        } catch {
          /* best-effort */
        }
  ...
  if (
    test('deleteAlias returns failure when saveAliases fails (read-only dir)', () => {
      if (process.platform === 'win32' || process.getuid?.() === 0) {
        console.log('    (skipped — chmod ineffective on Windows/root)');
        return;
      }
      ...
      try {
        ...
        assert.strictEqual(result.success, false, 'Should fail when save is blocked');
        assert.ok(
          result.error.includes('Failed to delete alias'),
  ...
  if (
    test('cleanupAliases returns failure when saveAliases fails after removing aliases', () => {
      if (process.platform === 'win32' || process.getuid?.() === 0) {
        console.log('    (skipped — chmod ineffective on Windows/root)');
        return;
      }
      ...
      try {
        ...
        assert.strictEqual(result.success, false, 'Should fail when save is blocked');
        assert.ok(
          result.error.includes('Failed to save after cleanup'),
        ...
        assert.ok(
          result.removedAliases.some((a) => a.name === 'remove-me'),
  ...
  if (
    test('setAlias returns failure when saveAliases fails', () => {
      if (process.platform === 'win32' || process.getuid?.() === 0) {
        console.log('    (skipped — chmod ineffective on Windows/root)');
        return;
      }
      ...
      try {
        ...
        assert.strictEqual(result.success, false, 'Should fail when save is blocked');
        assert.ok(
          result.error.includes('Failed to save alias'),
          `Should return save failure error, got: ${result.error}`
        );
      } finally {
        try {
  ...
  if (
    test('listAliases sorts entries with invalid/missing dates to the end via || 0 fallback', () => {
      ...
      assert.strictEqual(list[0].name, 'valid-alias', 'Entry with valid dates should sort first');
  ...
  if (
    test('loadAliases resets to defaults when aliases field is a string (typeof !== object)', () => {
  ...
  if (
    test('saveAliases triggers inner restoreErr catch when both save and restore fail', () => {
      ...
      if (process.platform === 'win32' || process.getuid?.() === 0) {
        console.log('    (skipped — chmod not reliable on Windows/root)');
        return;
      }
  ...
  if (
    test('renameAlias returns "already exists" error when renaming alias to itself', () => {
      resetAliases();
      ...
      assert.strictEqual(resolved.sessionPath, '/path/session', 'Alias data should be preserved');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('cleanupAliases removes alias when callback returns 0 (falsy coercion: !0 === true)', () => {
      resetAliases();
      ...
      assert.strictEqual(result.success, true, 'Cleanup should succeed');
      const resolved = aliases.resolveAlias('zero-test');
      assert.strictEqual(resolved, null, 'Alias should no longer exist after removal');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('setAlias with title=0 stores null (0 || null === null due to JavaScript falsy coercion)', () => {
  ...
  if (
    test('loadAliases accepts array aliases because typeof [] === "object" passes validation', () => {
      ...
      assert.strictEqual(data.aliases.length, 3, 'Array should have 3 elements');
  ...
  if (
    test('resolveSessionAlias returns path-traversal input as-is when alias lookup fails', () => {
  ...
  if (
    test('setAlias stores whitespace-only title as-is (no trim validation, unlike sessionPath)', () => {
      resetAliases();
  ...
  if (
    test('setAlias accepts alias of exactly 128 characters (128 is NOT > 128)', () => {
      ...
      assert.ok(result129.error.includes('128'), 'Error message should mention 128-char limit');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('resolveAlias returns null for alias names containing Unicode characters', () => {
      resetAliases();
      ...
      assert.strictEqual(umlautResult, null, 'Umlaut "ü" should be rejected by [a-zA-Z0-9_-]');

      // CJK characters
      const cjkResult = aliases.resolveAlias('会議-notes');
      assert.strictEqual(cjkResult, null, 'CJK characters should be rejected');

      // Emoji
      const emojiResult = aliases.resolveAlias('rocket-🚀');
      assert.strictEqual(emojiResult, null, 'Emoji should be rejected by the ASCII-only regex');
  ...
  if (
    test('listAliases throws TypeError when search option is a number (no toLowerCase method)', () => {
      resetAliases();
  ...
  if (
    test('updateAliasTitle with empty string stores null but returns empty string (|| coercion mismatch)', () => {
      resetAliases();
      ...
      assert.strictEqual(
        result.title,
        '',
        'Return value reflects the input parameter (empty string)'
      );
  ...
  if (
    test('loadAliases preserves extra unknown fields because only aliases key is validated', () => {
      resetAliases();
  ...
  if (
    test('renameAlias to the same name returns "already exists" error (no self-rename short-circuit)', () => {
      resetAliases();
  ...
  console.log('\nRound 118: setAlias (reserved names — case-insensitive rejection):');
  if (
    test('setAlias rejects all reserved names case-insensitively (list, help, remove, delete, create, set)', () => {
      resetAliases();
      ...
      for (const name of reserved) {
        ...
        assert.strictEqual(result.success, false, `'${name}' should be rejected as reserved`);
        assert.ok(
          result.error.includes('reserved'),
      ...
      assert.strictEqual(mixedResult.success, false, '"Help" (mixed case) should be rejected');

      const allCapsResult = aliases.setAlias('DELETE', '/path/to/session');
      assert.strictEqual(allCapsResult.success, false, '"DELETE" (all caps) should be rejected');
  ...
  if (
    test('renameAlias rejects reserved names for newAlias (same reserved list as setAlias)', () => {
      resetAliases();
      ...
      assert.ok(listResult.error.includes('reserved'), 'Error should mention "reserved"');
  ...
  if (
    test('setAlias accepts exactly 128-char alias name but rejects 129 chars (> 128 boundary)', () => {
      resetAliases();
      ...
      assert.ok(result129.error.includes('128'), 'Error should mention the 128 character limit');

      // 1 character — minimum valid
      const name1 = 'x';
      const result1 = aliases.setAlias(name1, '/path/to/session');
      assert.strictEqual(result1.success, true, 'Single character alias should be accepted');
  ...
  if (
    test('setAlias rejects invalid sessionPath: null, empty, whitespace-only, and non-string types', () => {
      resetAliases();
  ...
  if (
    test('listAliases limit=0 returns all aliases because 0 is falsy in JS (no slicing)', () => {
      resetAliases();
      aliases.setAlias('alias-a', '/path/a');
      aliases.setAlias('alias-b', '/path/b');
      ...
      assert.strictEqual(nanResult.length, 3, 'limit=NaN should return ALL aliases (NaN is falsy)');

      // limit=1: normal case — returns exactly 1
      const oneResult = aliases.listAliases({ limit: 1 });
      assert.strictEqual(oneResult.length, 1, 'limit=1 should return exactly 1 alias');

      // limit=2: returns exactly 2
      const twoResult = aliases.listAliases({ limit: 2 });
      assert.strictEqual(twoResult.length, 2, 'limit=2 should return exactly 2 aliases');

      // limit=100 (more than total): returns all 3
      const bigResult = aliases.listAliases({ limit: 100 });
      assert.strictEqual(bigResult.length, 3, 'limit > total should return all aliases');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('loadAliases with __proto__ alias key does not pollute Object prototype', () => {
  ...
  "metadata": { "totalCount": 2, "lastUpdated": "${now}" }
      ...
      assert.ok(
        data.aliases['__proto__'],
        '__proto__ key exists as normal property in parsed aliases'
      );
      assert.strictEqual(
        data.aliases['__proto__'].sessionPath,
        '/evil/path',
        '__proto__ alias data is accessible normally'
      );

      // Normal alias also works
      assert.ok(data.aliases['normal'], 'Normal alias coexists with __proto__ key');
      ...
      assert.ok(keys.includes('normal'), 'Object.keys includes normal alias');
    })
  )
    passed++;
  else failed++;
