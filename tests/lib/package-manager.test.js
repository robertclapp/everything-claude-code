...
function runTests() {
  ...
  if (
    test('PACKAGE_MANAGERS has all expected managers', () => {
  ...
  if (
    test('Each manager has required properties', () => {
      const requiredProps = [
        'name',
        'lockFile',
        'installCmd',
  ...
  if (
    test('handles packageManager without version', () => {
      const testDir = createTestDir();
      try {
        fs.writeFileSync(
          path.join(testDir, 'package.json'),
          JSON.stringify({ name: 'test', packageManager: 'yarn' })
        );
        const result = pm.detectFromPackageJson(testDir);
        assert.strictEqual(result, 'yarn');
      } finally {
  ...
  if (
    test('returns null when no packageManager field', () => {
      const testDir = createTestDir();
      try {
  ...
  if (
    test('returns array of available managers', () => {
  ...
  if (
    test('returns object with name, config, and source', () => {
  ...
  if (
    test('detects from lock file in project', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      delete process.env.CLAUDE_PACKAGE_MANAGER;
      const testDir = createTestDir();
      try {
  ...
  if (
    test('returns correct install command', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
  ...
  if (
    test('returns correct test command', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
  ...
  if (
    test('returns correct exec command for npm', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('returns correct exec command for pnpm', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'pnpm';
  ...
  if (
    test('generates pattern for dev command', () => {
  ...
  if (
    test('pattern matches actual commands', () => {
  ...
  if (
    test('returns informative prompt', () => {
  ...
  if (
    test('sets project package manager', () => {
      const testDir = createTestDir();
      try {
  ...
  if (
    test('rejects unknown package manager', () => {
      assert.throws(() => {
  ...
  if (
    test('handles invalid JSON in package.json', () => {
      const testDir = createTestDir();
      try {
  ...
  if (
    test('returns null for unknown package manager in packageManager field', () => {
      const testDir = createTestDir();
      try {
        fs.writeFileSync(
          path.join(testDir, 'package.json'),
          JSON.stringify({ name: 'test', packageManager: 'deno@1.0' })
        );
        const result = pm.detectFromPackageJson(testDir);
        assert.strictEqual(result, null);
      } finally {
  ...
  if (
    test('returns exec command without args', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('returns correct build command', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('returns correct dev command', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('returns correct custom script command', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('generates pattern for install command', () => {
  ...
  if (
    test('generates pattern for custom action', () => {
  ...
  if (
    test('falls through on corrupted project config JSON', () => {
  ...
  if (
    test('falls through on project config with unknown PM', () => {
      const testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pm-robust-'));
      const claudeDir = path.join(testDir, '.claude');
      fs.mkdirSync(claudeDir, { recursive: true });
      fs.writeFileSync(
  ...
  if (
    test('rejects empty script name', () => {
  ...
  if (
    test('rejects script name with shell metacharacters', () => {
  ...
  if (
    test('rejects script name with backticks', () => {
  ...
  if (
    test('rejects empty binary name', () => {
  ...
  if (
    test('rejects binary name with shell metacharacters', () => {
  ...
  if (
    test('accepts dotted binary names like tsc', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('detects from valid project-config (.claude/package-manager.json)', () => {
  ...
  if (
    test('project-config takes priority over package.json', () => {
      ...
      try {
        ...
        assert.strictEqual(
          result.name,
          'bun',
          'Project config should win over package.json and lock file'
        );
  ...
  if (
    test('package.json takes priority over lock file', () => {
  ...
  if (
    test('defaults to npm when no config found', () => {
  ...
  if (
    test('successfully saves preferred package manager', () => {
  ...
  if (
    test('generates pattern for test command', () => {
      const pattern = pm.getCommandPattern('test');
  ...
  if (
    test('generates pattern for build command', () => {
  ...
  if (
    test('pnpm custom script: pnpm (no run keyword)', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'pnpm';
  ...
  if (
    test('yarn custom script: yarn <script>', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'yarn';
  ...
  if (
    test('bun custom script: bun run <script>', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
  ...
  if (
    test('npm custom script: npm run <script>', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
        const cmd = pm.getRunCommand('lint');
  ...
  if (
    test('pnpm install returns pnpm install', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'pnpm';
  ...
  if (
    test('yarn install returns yarn (no install keyword)', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'yarn';
  ...
  if (
    test('bun test returns bun test', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'bun';
  ...
  if (
    test('pnpm exec: pnpm dlx <binary>', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'pnpm';
        assert.strictEqual(
          pm.getExecCommand('prettier', '--write .'),
  ...
  if (
    test('yarn exec: yarn dlx <binary>', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'yarn';
  ...
  if (
    test('ignores unknown env var package manager', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
  ...
  if (
    test('rejects args with shell metacharacter semicolon', () => {
  ...
  if (
    test('rejects args with pipe character', () => {
  ...
  if (
    test('rejects args with backtick injection', () => {
  ...
  if (
    test('rejects args with dollar sign', () => {
  ...
  if (
    test('rejects args with ampersand', () => {
      assert.throws(
        () => pm.getExecCommand('prettier', '--write . && echo pwned'),
        /unsafe characters/
      );
    })
  )
  ...
  if (
    test('allows safe args like --write .', () => {
      const cmd = pm.getExecCommand('prettier', '--write .');
  ...
  if (
    test('allows empty args without trailing space', () => {
      const cmd = pm.getExecCommand('prettier', '');
  ...
  if (
    test('escapes dot in action name for regex safety', () => {
      ...
      assert.ok(
        !regex.test('npm run testXall'),
        'Should NOT match arbitrary character in place of dot'
      );
    })
  )
    passed++;
  ...
  if (
    test('escapes brackets in action name', () => {
  ...
  if (
    test('escapes parentheses in action name', () => {
      // Should not throw when compiled as regex
      const pattern = pm.getCommandPattern('foo(bar)');
      assert.doesNotThrow(
        () => new RegExp(pattern),
        'Should produce valid regex with escaped parens'
      );
    })
  )
    passed++;
  ...
  if (
    test('rejects undefined script name', () => {
  ...
  if (
    test('rejects numeric script name', () => {
  ...
  if (
    test('rejects boolean script name', () => {
  ...
  if (
    test('rejects undefined binary name', () => {
  ...
  if (
    test('rejects numeric binary name', () => {
  ...
  if (
    test('escapes all regex metacharacters in action', () => {
  ...
  if (
    test('escapeRegex preserves alphanumeric chars', () => {
  ...
  if (
    test('ignores global config with non-string packageManager', () => {
  ...
  if (
    test('escapes pipe character in action name', () => {
  ...
  if (
    test('escapes dollar sign in action name', () => {
  ...
  if (
    test('handles action with leading/trailing spaces gracefully', () => {
  ...
  if (
    test('known action "dev" does NOT use escapeRegex path', () => {
  ...
  if (
    test('setProjectPackageManager creates .claude directory if missing', () => {
  ...
  if (
    test('setProjectPackageManager includes setAt timestamp', () => {
  ...
  if (
    test('allows colons in args (e.g. --fix:all)', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('allows at-sign in args (e.g. @latest)', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('allows equals in args (e.g. --config=path)', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('getExecCommand with args=0 produces command without extra args', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('getExecCommand with args=false produces command without extra args', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('getExecCommand with args=null produces command without extra args', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('detectFromPackageJson handles array packageManager field gracefully', () => {
  ...
  if (
    test('detectFromPackageJson handles numeric packageManager field gracefully', () => {
      const tmpDir = createTestDir();
      try {
  ...
  if (
    test('returns null for packageManager with non-@ separator', () => {
      const testDir = createTestDir();
      try {
        fs.writeFileSync(
          path.join(testDir, 'package.json'),
  ...
  if (
    test('extracts package manager from caret version like yarn@^4.0.0', () => {
      const testDir = createTestDir();
      try {
        fs.writeFileSync(
          path.join(testDir, 'package.json'),
  ...
  if (
    test('getPackageManager falls through corrupted global config to npm default', () => {
      const tmpDir = createTestDir();
  ...
  if (
    test('getPackageManager returns source global-config when valid global config exists', () => {
      const tmpDir = createTestDir();
      const projDir = path.join(tmpDir, 'proj');
      fs.mkdirSync(projDir, { recursive: true });
  ...
  if (
    test('setPreferredPackageManager throws wrapped error when save fails', () => {
  ...
  if (
    test('setProjectPackageManager throws wrapped error when write fails', () => {
      if (process.platform === 'win32' || process.getuid?.() === 0) {
        console.log(' (skipped — chmod ineffective on Windows/root)');
        return;
      }
  ...
  if (
    test('getExecCommand with args=42 (truthy number) appends stringified value', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('detectFromPackageJson returns null for empty (0-byte) package.json', () => {
      ...
      assert.strictEqual(
        result,
        null,
        'Empty package.json should return null (content="" is falsy)'
      );
      cleanupTestDir(testDir);
    })
  )
  ...
  if (
    test('getCommandPattern with empty string returns valid regex pattern', () => {
  ...
  if (
    test('detectFromPackageJson returns null for whitespace-only packageManager field', () => {
      // package-manager.js line 114-119: \" \" is truthy, so enters the if block.
      // \" \".split('@')[0] = \" \" which doesn't match any PACKAGE_MANAGERS key.
      const testDir = createTestDir();
      fs.writeFileSync(path.join(testDir, 'package.json'), JSON.stringify({ packageManager: ' ' }));
      const result = pm.detectFromPackageJson(testDir);
  ...
  if (
    test('detectFromPackageJson returns null for scoped package name (@scope/pkg@version)', () => {
  ...
  if (
    test('getPackageManager skips empty string CLAUDE_PACKAGE_MANAGER (falsy short-circuit)', () => {
  ...
  if (
    test('detectFromLockFile(null) throws TypeError (path.join rejects null)', () => {
  ...
  if (
    test('getExecCommand with args={} bypasses SAFE_ARGS validation and coerces to "[object Object]"', () => {
  ...
  if (
    test('getExecCommand accepts ../../../etc/passwd as binary because SAFE_NAME_REGEX allows ../', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('getRunCommand accepts @scope/../../evil because SAFE_NAME_REGEX allows ../', () => {
      const originalEnv = process.env.CLAUDE_PACKAGE_MANAGER;
      try {
        process.env.CLAUDE_PACKAGE_MANAGER = 'npm';
  ...
  if (
    test('getExecCommand accepts newline in args because SAFE_ARGS_REGEX includes newline', () => {
      ...
      try {
        ...
        assert.strictEqual(
          ...
          'Newline passes SAFE_ARGS_REGEX'
        );
        // Tab also passes
