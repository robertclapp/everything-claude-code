...
function runTests() {
  ...
  if (
    test('isWindows/isMacOS/isLinux are booleans', () => {
  ...
  if (
    test('exactly one platform should be true', () => {
  ...
  if (
    test('getClaudeDir returns path under home', () => {
  ...
  if (
    test('getSessionsDir returns path under Claude dir', () => {
  ...
  if (
    test('getTempDir returns valid temp directory', () => {
  ...
  if (
    test('getDateTimeString returns full datetime format', () => {
      const dt = utils.getDateTimeString();
      assert.ok(
        /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(dt),
        `Expected YYYY-MM-DD HH:MM:SS, got ${dt}`
      );
    })
  )
    passed++;
  ...
  if (
    test('getGitRepoName returns string or null', () => {
  ...
  if (
    test('getProjectName returns non-empty string', () => {
  ...
  if (
    test('getSessionIdShort falls back to project name', () => {
  ...
  if (
    test('getSessionIdShort returns last 8 characters', () => {
      const original = process.env.CLAUDE_SESSION_ID;
  ...
  if (
    test('getSessionIdShort handles short session IDs', () => {
      const original = process.env.CLAUDE_SESSION_ID;
  ...
  if (
    test('replaceInFile replaces text', () => {
      const testFile = path.join(utils.getTempDir(), `utils-test-${Date.now()}.txt`);
      try {
  ...
  if (
    test('countInFile counts occurrences', () => {
      const testFile = path.join(utils.getTempDir(), `utils-test-${Date.now()}.txt`);
      try {
  ...
  if (
    test('grepFile finds matching lines', () => {
      const testFile = path.join(utils.getTempDir(), `utils-test-${Date.now()}.txt`);
      try {
  ...
  if (
    test('findFiles returns empty for non-existent directory', () => {
  ...
  if (
    test('findFiles finds matching files', () => {
      const testDir = path.join(utils.getTempDir(), `utils-test-${Date.now()}`);
      try {
  ...
  if (
    test('findFiles returns empty for null/undefined dir', () => {
  ...
  if (
    test('findFiles returns empty for null/undefined pattern', () => {
  ...
  if (
    test('findFiles supports maxAge filter', () => {
  ...
  if (
    test('findFiles supports recursive option', () => {
  ...
  if (
    test('countInFile handles invalid regex pattern', () => {
      const testFile = path.join(utils.getTempDir(), `utils-test-${Date.now()}.txt`);
      try {
  ...
  if (
    test('countInFile handles non-string non-regex pattern', () => {
      const testFile = path.join(utils.getTempDir(), `utils-test-${Date.now()}.txt`);
      try {
        utils.writeFile(testFile, 'test content');
  ...
  if (
    test('countInFile enforces global flag on RegExp', () => {
      const testFile = path.join(utils.getTempDir(), `utils-test-${Date.now()}.txt`);
      try {
        utils.writeFile(testFile, 'foo bar foo baz foo');
  ...
  if (
    test('grepFile handles invalid regex pattern', () => {
      const testFile = path.join(utils.getTempDir(), `utils-test-${Date.now()}.txt`);
      try {
        utils.writeFile(testFile, 'test content');
  ...
  if (
    test('replaceInFile returns false for non-existent file', () => {
  ...
  if (
    test('countInFile returns 0 for non-existent file', () => {
  ...
  if (
    test('grepFile returns empty for non-existent file', () => {
  ...
  if (
    test('commandExists rejects unsafe command names', () => {
  ...
  if (
    test('runCommand executes simple command', () => {
  ...
  if (
    test('runCommand handles failed command', () => {
  ...
  if (
    test('output() writes string to stdout', () => {
  ...
  if (
    test('output() JSON-stringifies objects', () => {
      let captured = null;
      const origLog = console.log;
      console.log = (v) => {
        captured = v;
  ...
  if (
    test('output() JSON-stringifies null (typeof null === "object")', () => {
      let captured = null;
      const origLog = console.log;
      console.log = (v) => {
        captured = v;
  ...
  if (
    test('output() handles arrays as objects', () => {
      let captured = null;
      const origLog = console.log;
      console.log = (v) => {
        captured = v;
  ...
  if (
    test('getGitModifiedFiles returns an array', () => {
  ...
  if (
    test('getGitModifiedFiles filters by regex patterns', () => {
  ...
  if (
    test('getGitModifiedFiles skips invalid patterns', () => {
  ...
  if (
    test('getGitModifiedFiles skips non-string patterns', () => {
  ...
  if (
    test('getLearnedSkillsDir returns path under Claude dir', () => {
  ...
  if (
    test('replaces first match when regex has no g flag', () => {
      const testFile = path.join(utils.getTempDir(), `utils-test-${Date.now()}.txt`);
      try {
        utils.writeFile(testFile, 'foo bar foo baz foo');
  ...
  if (
    test('replaces all matches when regex has g flag', () => {
      const testFile = path.join(utils.getTempDir(), `utils-test-${Date.now()}.txt`);
      try {
        utils.writeFile(testFile, 'foo bar foo baz foo');
  ...
  if (
    test('replaces with string search (first occurrence)', () => {
      const testFile = path.join(utils.getTempDir(), `utils-test-${Date.now()}.txt`);
      try {
  ...
  if (
    test('replaces all occurrences with string when options.all is true', () => {
      const testFile = path.join(utils.getTempDir(), `utils-test-${Date.now()}.txt`);
      try {
  ...
  if (
    test('options.all is ignored for regex patterns', () => {
      const testFile = path.join(utils.getTempDir(), `utils-test-${Date.now()}.txt`);
      try {
        ...
        assert.strictEqual(
          content,
          'qux bar foo',
          'Regex without g should still replace first only'
        );
      } finally {
        fs.unlinkSync(testFile);
      }
  ...
  if (
    test('replaces with capture groups', () => {
      const testFile = path.join(utils.getTempDir(), `utils-test-${Date.now()}.txt`);
      try {
  ...
  if (
    test('writeFile handles unicode content', () => {
      const testFile = path.join(utils.getTempDir(), `utils-test-${Date.now()}.txt`);
      try {
  ...
  if (
    test('findFiles handles regex special chars in pattern', () => {
  ...
  if (
    test('findFiles wildcard still works with special chars', () => {
  ...
  const stdinScript =
    'const u=require("./scripts/lib/utils");u.readStdinJson({timeoutMs:2000}).then(d=>{process.stdout.write(JSON.stringify(d))})';
  const stdinOpts = { encoding: 'utf8', cwd: path.join(__dirname, '..', '..'), timeout: 5000 };

  if (
    test('readStdinJson parses valid JSON from stdin', () => {
      const { execFileSync } = require('child_process');
      const result = execFileSync('node', ['-e', stdinScript], {
        ...stdinOpts,
  ...
  if (
    test('readStdinJson returns {} for invalid JSON', () => {
      const { execFileSync } = require('child_process');
  ...
  if (
    test('readStdinJson handles nested objects', () => {
      const { execFileSync } = require('child_process');
      const result = execFileSync('node', ['-e', stdinScript], {
        ...stdinOpts,
        input: '{"a":{"b":1},"c":[1,2]}',
  ...
  if (
    test('grepFile with /g flag finds ALL matching lines (not alternating)', () => {
  ...
  if (
    test('grepFile preserves regex flags other than g (e.g. case-insensitive)', () => {
      ...
      try {
        ...
        assert.strictEqual(
          matches.length,
          3,
          `Should find 3 case-insensitive matches, found ${matches.length}`
        );
      } finally {
        fs.unlinkSync(testFile);
      }
  ...
  if (
    test('commandExists rejects empty string', () => {
      assert.strictEqual(
        utils.commandExists(''),
        false,
  ...
  if (
    test('commandExists rejects command with spaces', () => {
      assert.strictEqual(
        utils.commandExists('my command'),
        false,
        'Commands with spaces should be rejected'
      );
    })
  )
    passed++;
  ...
  if (
    test('commandExists rejects command with path separators', () => {
      assert.strictEqual(
        utils.commandExists('/usr/bin/node'),
        false,
        'Commands with / should be rejected'
      );
      assert.strictEqual(
        utils.commandExists('..\\cmd'),
        false,
        'Commands with \\ should be rejected'
      );
    })
  ...
  if (
    test('commandExists rejects shell metacharacters', () => {
      assert.strictEqual(utils.commandExists('cmd;ls'), false, 'Semicolons should be rejected');
      assert.strictEqual(
        utils.commandExists('$(whoami)'),
        false,
        'Subshell syntax should be rejected'
      );
  ...
  if (
    test('commandExists allows dots and underscores', () => {
  ...
  if (
    test('findFiles with ? wildcard matches single character', () => {
      ...
      try {
        ...
        assert.deepStrictEqual(
          names,
          ['a1.txt', 'b2.txt'],
          'Should match exactly 2-char basenames'
        );
      } finally {
        fs.rmSync(testDir, { recursive: true, force: true });
      }
  ...
  if (
    test('findFiles sorts by mtime (newest first)', () => {
  ...
  if (
    test('findFiles with maxAge filters old files', () => {
  ...
  if (
    test('ensureDir is safe for concurrent calls (EEXIST race)', () => {
  ...
  if (
    test('ensureDir returns the directory path', () => {
  ...
  if (
    test('runCommand returns trimmed output', () => {
  ...
  if (
    test('getGitModifiedFiles returns array with empty patterns', () => {
      const files = utils.getGitModifiedFiles([]);
  ...
  if (
    test('readStdinJson type check: returns a Promise', () => {
  ...
  if (
    test('readStdinJson maxSize stops accumulating after threshold (chunk-level guard)', () => {
      ...
      const script =
        'const u=require("./scripts/lib/utils");u.readStdinJson({timeoutMs:2000,maxSize:100}).then(d=>{process.stdout.write(JSON.stringify(d))})';
  ...
  if (
    test('readStdinJson with maxSize large enough preserves valid JSON', () => {
      const { execFileSync } = require('child_process');
      const script =
        'const u=require("./scripts/lib/utils");u.readStdinJson({timeoutMs:2000,maxSize:1024}).then(d=>{process.stdout.write(JSON.stringify(d))})';
  ...
  if (
    test('readStdinJson resolves {} for whitespace-only stdin', () => {
      const { execFileSync } = require('child_process');
      const result = execFileSync('node', ['-e', stdinScript], {
        ...stdinOpts,
        input: '   \n  \t  ',
  ...
  if (
    test('readStdinJson handles JSON with trailing whitespace/newlines', () => {
      const { execFileSync } = require('child_process');
      const result = execFileSync('node', ['-e', stdinScript], {
        ...stdinOpts,
        input: '{"a":1}  \n\n',
  ...
  if (
    test('readStdinJson handles JSON with BOM prefix (returns {})', () => {
      const { execFileSync } = require('child_process');
  ...
  if (
    test('ensureDir wraps non-EEXIST errors with descriptive message', () => {
  ...
  if (
    test('ensureDir error includes the directory path', () => {
  ...
  if (
    test('runCommand returns stderr content on failure when stderr exists', () => {
      const result = utils.runCommand(
        'node -e "process.stderr.write(\'custom error\'); process.exit(1)"'
      );
      assert.strictEqual(result.success, false);
  ...
  if (
    test('runCommand falls back to err.message when no stderr', () => {
  ...
  if (
    test('readStdinJson resolves {} when stdin emits error (via broken pipe)', () => {
  ...
  if (
    test('readStdinJson error handler is guarded by settled flag', () => {
      ...
      assert.strictEqual(
        parsed.test,
        'settled-guard',
        'Should parse normally when end fires first'
      );
    })
  )
    passed++;
  ...
  if (
    test('replaceInFile returns false on write failure (read-only file)', () => {
  ...
  if (
    test('findFiles recursive scan skips unreadable subdirectory silently', () => {
      if (process.platform === 'win32' || process.getuid?.() === 0) {
        console.log('    (skipped — chmod ineffective on Windows/root)');
        return;
      }
      ...
      try {
        ...
        assert.ok(
          !paths.some((p) => p.includes('hidden.txt')),
          'Should not find unreadable/hidden.txt'
        );
      } finally {
  ...
  if (
    test('countInFile counts occurrences using a plain string pattern', () => {
  ...
  if (
    test('grepFile finds matching lines using a plain string pattern', () => {
  ...
  if (
    test('findFiles skips broken symlinks that match the pattern', () => {
      ...
      try {
        ...
        assert.ok(
          !paths.some((p) => p.includes('broken.txt')),
  ...
  if (
    test('getSessionIdShort uses fallback when getProjectName returns null (CWD at root)', () => {
      if (process.platform === 'win32') {
    ...
    `;
      ...
      assert.strictEqual(
        result.status,
        0,
        `Should exit 0, got status ${result.status}. stderr: ${result.stderr}`
      );
      assert.strictEqual(
        result.stdout,
        'my-custom-fallback',
  ...
  if (
    test('replaceInFile with empty string replacement deletes matched text', () => {
  ...
  if (
    test('countInFile returns 0 for valid file with no pattern matches', () => {
  ...
  if (
    test('countInFile returns 0 for object pattern (neither string nor RegExp)', () => {
  ...
  if (
    test('countInFile with /pattern/i appends g flag and counts case-insensitively', () => {
  ...
  if (
    test('countInFile with /pattern/gi preserves existing flags and counts correctly', () => {
  ...
  if (
    test('countInFile with /apple|banana/ (alternation, no g) counts all matches', () => {
  ...
  if (
    test('getSessionIdShort returns whitespace when CLAUDE_SESSION_ID is all spaces', () => {
  ...
  if (
    test('countInFile returns consistent count when same RegExp object is reused', () => {
  ...
  if (
    test('findFiles with maxAge: -1 excludes all files (ageInDays always >= 0)', () => {
  ...
  if (
    test('replaceInFile returns true and rewrites file even when search does not match', () => {
  ...
  if (
    test('grepFile treats CR-only file as a single line (splits on \\n only)', () => {
  ...
  if (
    test('findFiles with maxAge AND recursive filters age across subdirectories', () => {
  ...
  if (
    test('output() throws TypeError on circular reference object (JSON.stringify has no try/catch)', () => {
      const circular = { a: 1 };
  ...
  if (
    test('countInFile returns 0 for boolean false pattern (else branch at line 443)', () => {
  ...
  if (
    test('grepFile with numeric 0 implicitly coerces to /0/ via RegExp constructor', () => {
  ...
  if (
    test('grepFile with /pattern/y sticky flag misses lines due to lastIndex state', () => {
  ...
  if (
    test('grepFile matches empty lines with ^$ pattern including trailing newline phantom line', () => {
      ...
      try {
        ...
        assert.strictEqual(
          ...
          5,
          'Third empty line is the trailing phantom from split'
        );
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
  ...
  if (
    test('replaceInFile does not re-scan replacement text (single-pass, no infinite loop)', () => {
  ...
  if (
    test('countInFile with named capture groups counts matches not groups', () => {
  ...
  if (
    test('grepFile preserves multiline (m) flag and anchors work on split lines', () => {
  ...
  if (
    test('appendFile creates parent directory and new file when neither exist', () => {
      ...
      try {
        ...
        assert.ok(
          !fs.existsSync(path.join(tmpDir, 'deep')),
  ...
  if (
    test('grepFile finds Unicode emoji patterns across lines', () => {
      ...
      try {
        ...
        assert.ok(
          cjkResults[0].content.includes('日本語テスト'),
  ...
  if (
    test('findFiles returns empty array when root directory exists but is unreadable', () => {
      if (process.platform === 'win32' || process.getuid?.() === 0) {
        console.log('    (skipped — chmod ineffective on Windows/root)');
        return true;
      }
  ...
  if (
    test('replaceInFile with zero-width regex /(?:)/g inserts replacement at every position', () => {
  ...
  if (
    test('replaceInFile ignores options.all when search is a RegExp — falls through to .replace()', () => {
  ...
  if (
    test('output throws TypeError when object contains BigInt values (JSON.stringify cannot serialize)', () => {
      ...
      try {
        ...
        assert.strictEqual(
          captured,
          BigInt(42),
          'Plain BigInt goes to else branch, logged directly'
        );
  ...
  if (
    test('countInFile with empty string pattern returns content.length + 1 (matches between every char)', () => {
  ...
  if (
    test('grepFile with CRLF content: unanchored patterns work but anchored $ fails due to trailing \\r', () => {
      ...
      try {
        ...
        assert.ok(
          unanchored[0].content.endsWith('\r'),
  ...
  if (
    test('replaceInFile with null replacement coerces to string "null" via String.replace ToString', () => {
  ...
  if (
    test('ensureDir with null path throws wrapped Error from TypeError (ERR_INVALID_ARG_TYPE)', () => {
  ...
  if (
    test('writeFile with null/number content throws TypeError because fs.writeFileSync rejects non-string data', () => {
  ...
  if (
    test('appendFile with null/number content throws TypeError (no try/catch wrapper)', () => {
  ...
  if (
    test('replaceInFile with empty search: replace prepends at pos 0; replaceAll inserts between every char', () => {
  ...
  if (
    test('findFiles with ? glob matches single character only — test?.txt matches test1 but not test12', () => {
  ...
  if (
    test('findFiles escapes dots in glob pattern so *.txt only matches literal .txt extension', () => {
  ...
  if (
    test('countInFile counts non-overlapping matches only — "aaa" with /aa/g returns 1 not 2', () => {
  ...
  if (
    test('replaceInFile replacement string interprets $& as matched text and $$ as literal $', () => {
  ...
  if (
    test('findFiles with * pattern matches dotfiles because .* regex includes hidden files', () => {
  ...
  if (
    test('readFile with binary content returns garbled string (not null) because UTF-8 decode does not throw', () => {
  ...
  if (
    test('output() handles undefined, NaN, Infinity as non-objects — logs directly', () => {
