...
function runTests() {
  ...
  if (
    test('getClawDir() returns path ending in .claude/claw', () => {
      const dir = getClawDir();
      assert.ok(
        dir.endsWith(path.join('.claude', 'claw')),
  ...
  if (
    test('getSessionPath("foo") returns correct .md path', () => {
      const p = getSessionPath('foo');
      assert.ok(
        p.endsWith(path.join('.claude', 'claw', 'foo.md')),
  ...
  if (
    test('listSessions() returns empty array for empty dir', () => {
  ...
  if (
    test('listSessions() finds .md files and strips extension', () => {
      const tmpDir = makeTmpDir();
      try {
  ...
  if (
    test('loadHistory() returns "" for non-existent file', () => {
  ...
  if (
    test('appendTurn() writes correct markdown format', () => {
      const tmpDir = makeTmpDir();
      ...
      try {
        ...
        assert.ok(
          content.includes('### [2025-01-15T10:00:00.000Z] User'),
  ...
  if (
    test('loadECCContext() concatenates multiple skill files', () => {
      ...
      for (const name of available.slice(0, 2)) {
        const skillContent = fs.readFileSync(path.join(skillsDir, name, 'SKILL.md'), 'utf8');
        // Check that at least part of each skill is present
        const firstLine = skillContent.split('\n').find((l) => l.trim().length > 10);
        if (firstLine) {
          assert.ok(result.includes(firstLine.trim()), `Should include content from skill ${name}`);
        }
      }
    })
  )
  ...
  if (
    test('buildPrompt() constructs correct prompt structure', () => {
  ...
  if (
    test('/clear truncates session file', () => {
      const tmpDir = makeTmpDir();
      const filePath = path.join(tmpDir, 'session.md');
      try {
        fs.writeFileSync(filePath, 'some existing history content');
        assert.ok(
          fs.readFileSync(filePath, 'utf8').length > 0,
          'File should have content before clear'
        );
  ...
  if (
    test('isValidSessionName rejects invalid characters', () => {
