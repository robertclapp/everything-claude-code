...
function runTests() {
  ...
  if (
    test('creates instance with repo name', () => {
  ...
  if (
    test('accepts custom width option', () => {
  ...
  if (
    test('outputs header with repo name', () => {
  ...
  if (
    test('header handles long repo names without crash', () => {
      const output = new SkillCreateOutput(
        'a-very-long-repository-name-that-exceeds-normal-width-limits'
      );
  ...
  if (
    test('displays patterns with confidence bars', () => {
      const output = new SkillCreateOutput('repo');
      const logs = captureLog(() =>
        output.patterns([
          {
  ...
  if (
    test('handles patterns with missing confidence', () => {
      const output = new SkillCreateOutput('repo');
      // Should default to 0.8 confidence
      const logs = captureLog(() =>
        output.patterns([{ name: 'No Confidence', trigger: 'always', evidence: 'evidence' }])
      );
      const combined = logs.join('\n');
  ...
  if (
    test('displays instincts in a box', () => {
      const output = new SkillCreateOutput('repo');
      const logs = captureLog(() =>
        output.instincts([
  ...
  if (
    test('displays file paths', () => {
      const output = new SkillCreateOutput('repo');
      const logs = captureLog(() => output.output('/path/to/SKILL.md', '/path/to/instincts.yaml'));
      const combined = logs.join('\n');
  ...
  if (
    test('displays next steps with commands', () => {
      const output = new SkillCreateOutput('repo');
  ...
  if (
    test('displays footer with attribution', () => {
      const output = new SkillCreateOutput('repo');
  ...
  if (
    test('does not crash with confidence > 1.0 (percent > 100)', () => {
      const output = new SkillCreateOutput('repo');
  ...
  if (
    test('renders 0% confidence bar without crash', () => {
      const output = new SkillCreateOutput('repo');
      const logs = captureLog(() =>
        output.patterns([
  ...
  if (
    test('renders 100% confidence bar without crash', () => {
      const output = new SkillCreateOutput('repo');
      const logs = captureLog(() =>
        output.patterns([
  ...
  if (
    test('patterns() with empty array produces header but no entries', () => {
      const output = new SkillCreateOutput('repo');
  ...
  if (
    test('instincts() with empty array produces box but no entries', () => {
      const output = new SkillCreateOutput('repo');
  ...
  if (
    test('box does not crash on title longer than width', () => {
      ...
      const logs = captureLog(() =>
        output.instincts([{ name: 'a-very-long-instinct-name', confidence: 0.9 }])
      );
  ...
  if (
    test('analysisResults does not crash with very narrow width', () => {
  ...
  if (
    test('top, middle, and bottom lines have equal visual width', () => {
      const output = new SkillCreateOutput('repo', { width: 40 });
      const logs = captureLog(() => output.instincts([{ name: 'test', confidence: 0.9 }]));
      const combined = logs.join('\n');
  ...
  if (
    test('box does not crash when content line exceeds width', () => {
      const output = new SkillCreateOutput('repo', { width: 30 });
      // Force a very long instinct name that exceeds width
      const logs = captureLog(() =>
        output.instincts([
          {
            name: 'this-is-an-extremely-long-instinct-name-that-clearly-exceeds-width',
            confidence: 0.9,
          },
        ])
      );
  ...
  if (
    test('patterns renders negative confidence without crash', () => {
      const output = new SkillCreateOutput('repo');
  ...
  if (
    test('header does not crash with very long repo name', () => {
  ...
  if (
    test('stripAnsi handles nested ANSI codes with multi-digit params', () => {
  ...
  if (
    test('header subtitle line matches border width', () => {
      const output = new SkillCreateOutput('test-repo');
      const logs = captureLog(() => output.header());
  ...
  if (
    test('header all lines have consistent width for short repo name', () => {
      ...
      widths.forEach((w, i) => {
        assert.strictEqual(w, first, `Line ${i} width (${w}) should match first line (${first})`);
      });
    })
  )
    passed++;
  ...
  if (
    test('header subtitle has correct content area width of 64 chars', () => {
  ...
  if (
    test('header subtitle line does not truncate with medium-length repo name', () => {
  ...
  if (
    test('box lines in instincts() match the default box width of 60', () => {
      const output = new SkillCreateOutput('repo');
      const logs = captureLog(() =>
        output.instincts([{ name: 'test-instinct', confidence: 0.85 }])
      );
      const combined = logs.join('\n');
      const boxLines = combined.split('\n').filter((l) => {
        const s = stripAnsi(l).trim();
      ...
      boxLines.forEach((l, i) => {
        ...
        assert.strictEqual(w, 60, `Box line ${i} should be 60 chars wide, got ${w}`);
      });
    })
  )
    passed++;
  ...
  if (
    test('box lines with custom width match the requested width', () => {
      const output = new SkillCreateOutput('repo', { width: 40 });
      const logs = captureLog(() => output.instincts([{ name: 'short', confidence: 0.9 }]));
      const combined = logs.join('\n');
      const boxLines = combined.split('\n').filter((l) => {
        const s = stripAnsi(l).trim();
        return s.startsWith('\u256D') || s.startsWith('\u2502') || s.startsWith('\u2570');
  ...
  if (
    test('analysisResults box lines are all 60 chars wide', () => {
      const output = new SkillCreateOutput('repo');
      const logs = captureLog(() =>
        output.analysisResults({
          commits: 50,
      ...
      boxLines.forEach((l, i) => {
        const w = stripAnsi(l).length;
        assert.strictEqual(w, 60, `Analysis box line ${i} should be 60 chars, got ${w}`);
      });
    })
  )
    passed++;
  ...
  if (
    test('nextSteps box lines are all 60 chars wide', () => {
      const output = new SkillCreateOutput('repo');
      const logs = captureLog(() => output.nextSteps());
      const combined = logs.join('\n');
      const boxLines = combined.split('\n').filter((l) => {
      ...
      boxLines.forEach((l, i) => {
        const w = stripAnsi(l).length;
        assert.strictEqual(w, 60, `NextSteps box line ${i} should be 60 chars, got ${w}`);
      });
    })
  )
    passed++;
  ...
  if (
    test('analysisResults handles zero values for all data fields', () => {
      const output = new SkillCreateOutput('repo');
      const logs = captureLog(() =>
        output.analysisResults({
          commits: 0,
  ...
  if (
    test('module exports demo function alongside SkillCreateOutput', () => {
      ...
      assert.strictEqual(
        typeof mod.SkillCreateOutput,
        'function',
        'SkillCreateOutput should be a constructor'
      );
    })
  )
    passed++;
  ...
  if (
    test('patterns() with confidence=0 shows 0%, not 80% (nullish coalescing fix)', () => {
      const output = new SkillCreateOutput('repo');
      const logs = captureLog(() =>
        output.patterns([
  ...
  if (
    test('analyzePhase completes without error and writes to stdout', () => {
      const output = new SkillCreateOutput('test-repo');
      ...
      assert.ok(
        writes.some((w) => w.includes('Analyzing')),
        'Should include "Analyzing" label'
      );
    })
  )
    passed++;
