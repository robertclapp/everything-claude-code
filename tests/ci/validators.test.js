...
function runTests() {
  ...
  if (
    test('passes on valid agent with all required fields', () => {
      const testDir = createTestDir();
      fs.writeFileSync(
        path.join(testDir, 'good-agent.md'),
        '---\nmodel: sonnet\ntools: Read, Write\n---\n# Agent'
      );
  ...
  if (
    test('handles frontmatter with colons in values', () => {
      const testDir = createTestDir();
      fs.writeFileSync(
        path.join(testDir, 'colon-agent.md'),
    ...
    test('fails on invalid async field type', () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          hooks: {
            PreToolUse: [
              { matcher: 'test', hooks: [{ type: 'command', command: 'echo', async: 'yes' }] },
            ],
          },
        })
      );
    ...
    test('fails on negative timeout', () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          hooks: {
            PreToolUse: [
              { matcher: 'test', hooks: [{ type: 'command', command: 'echo', timeout: -5 }] },
            ],
          },
        })
      );
    ...
    test('fails on invalid inline JS syntax', () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          hooks: {
            PreToolUse: [
              { matcher: 'test', hooks: [{ type: 'command', command: 'node -e "function {"' }] },
            ],
          },
        })
      );
  ...
  if (
    test('validates legacy array format', () => {
      ...
      fs.writeFileSync(
        hooksFile,
        JSON.stringify([{ matcher: 'test', hooks: [{ type: 'command', command: 'echo ok' }] }])
      );
  ...
  if (
    test('checks references in unclosed code blocks', () => {
      ...
      assert.ok(
        result.stderr.includes('phantom-cmd'),
        'Should report broken ref from unclosed block'
      );
      cleanupTestDir(testDir);
      cleanupTestDir(agentsDir);
      cleanupTestDir(skillsDir);
  ...
  if (
    test('rejects agent with invalid model value', () => {
      const testDir = createTestDir();
      fs.writeFileSync(
        path.join(testDir, 'bad-model.md'),
        '---\nmodel: gpt-4\ntools: Read\n---\n# Agent'
      );
  ...
  if (
    test('reports all invalid agents in mixed agent references', () => {
      ...
      const skillsDir = createTestDir();
      fs.writeFileSync(
        path.join(agentsDir, 'real-agent.md'),
        '---\nmodel: sonnet\ntools: Read\n---\n# A'
      );
      fs.writeFileSync(
        path.join(testDir, 'cmd.md'),
  ...
  if (
    test('validates workflow with hyphenated agent names', () => {
      ...
      const skillsDir = createTestDir();
      fs.writeFileSync(
        path.join(agentsDir, 'tdd-guide.md'),
        '---\nmodel: sonnet\ntools: Read\n---\n# T'
      );
      fs.writeFileSync(
  ...
  if (
    test('rejects top-level data that is a string', () => {
      ...
      assert.ok(
        result.stderr.includes('must be an object or array'),
        'Should report must be object or array'
      );
      cleanupTestDir(testDir);
    })
  )
    ...
    test('rejects array command with non-string elements', () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          hooks: {
            PreToolUse: [
              { matcher: 'test', hooks: [{ type: 'command', command: ['node', 123, null] }] },
            ],
          },
        })
      );
    ...
    test('rejects non-number timeout type', () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          hooks: {
            PreToolUse: [
              { matcher: 'test', hooks: [{ type: 'command', command: 'echo', timeout: 'fast' }] },
            ],
          },
        })
      );
    ...
    test('accepts timeout of exactly 0', () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          hooks: {
            PreToolUse: [
              { matcher: 'test', hooks: [{ type: 'command', command: 'echo', timeout: 0 }] },
            ],
          },
        })
      );
  ...
  if (
    test('legacy format: rejects matcher missing matcher field', () => {
      ...
      fs.writeFileSync(
        hooksFile,
        JSON.stringify([{ hooks: [{ type: 'command', command: 'echo ok' }] }])
      );
    ...
    test('rejects array command with empty string element', () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          hooks: {
            PreToolUse: [
              { matcher: 'test', hooks: [{ type: 'command', command: ['node', '', 'script.js'] }] },
            ],
          },
        })
      );
    ...
    test('rejects negative timeout', () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          hooks: {
            PreToolUse: [
              { matcher: 'test', hooks: [{ type: 'command', command: 'echo hi', timeout: -5 }] },
            ],
          },
        })
      );
    ...
    test('rejects non-boolean async field', () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          hooks: {
            PostToolUse: [
              { matcher: 'test', hooks: [{ type: 'command', command: 'echo ok', async: 'yes' }] },
            ],
          },
        })
      );
        ...
        JSON.stringify({
          hooks: {
            PreToolUse: [
              {
                matcher: 'test',
                hooks: [{ type: 'command', command: 'node -e "const x = 1 + 2; process.exit(0)"' }],
              },
            ],
          },
        })
  ...
  if (
    test('validates multiple command refs on same non-creates line', () => {
      ...
      assert.strictEqual(
        result.code,
        0,
        'Should pass when multiple refs on same line are all valid'
      );
      cleanupTestDir(testDir);
      cleanupTestDir(agentsDir);
      cleanupTestDir(skillsDir);
  ...
  if (
    test('passes when command has no slash references at all', () => {
      ...
      const skillsDir = createTestDir();
      fs.writeFileSync(
        path.join(testDir, 'cmd-simple.md'),
  ...
  if (
    test('accepts all valid model values (haiku, sonnet, opus)', () => {
      const testDir = createTestDir();
      fs.writeFileSync(
        path.join(testDir, 'haiku.md'),
        '---\nmodel: haiku\ntools: Read\n---\n# Haiku Agent'
      );
      fs.writeFileSync(
        path.join(testDir, 'sonnet.md'),
        '---\nmodel: sonnet\ntools: Read, Write\n---\n# Sonnet Agent'
      );
      fs.writeFileSync(
        path.join(testDir, 'opus.md'),
        '---\nmodel: opus\ntools: Read, Write, Bash\n---\n# Opus Agent'
      );
  ...
  if (
    test('flags agent path references when AGENTS_DIR does not exist', () => {
      ...
      assert.strictEqual(
        result.code,
        1,
        'Should fail when agents dir missing but agent referenced'
      );
      assert.ok(
        result.stderr.includes('planner'),
        'Should report the unresolvable agent reference'
      );
      cleanupTestDir(testDir);
      cleanupTestDir(skillsDir);
    })
            ...
            PreToolUse: [
              {
                matcher: 'test',
                hooks: [
                  { type: 'command', command: 'node -e "var a = \\"ok\\"\\nconsole.log(a)"' },
                ],
              },
            ],
          },
    ...
    test('rejects inline JS with syntax error after unescaping', () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          hooks: {
            PreToolUse: [
              { matcher: 'test', hooks: [{ type: 'command', command: 'node -e "var x = {"' }] },
            ],
          },
        })
      );
  ...
  if (
    test('validates workflow arrows with irregular whitespace', () => {
      ...
      assert.strictEqual(
        result.code,
        0,
        'Workflow arrows with irregular whitespace should be valid'
      );
      cleanupTestDir(testDir);
      cleanupTestDir(agentsDir);
      cleanupTestDir(skillsDir);
  ...
  if (
    test('passes rule file containing only a fenced code block', () => {
      const testDir = createTestDir();
      fs.writeFileSync(
        path.join(testDir, 'code-only.md'),
        '```javascript\nfunction example() {\n  return true;\n}\n```'
      );
  ...
  if (
    test('strips multiple adjacent code blocks before checking references', () => {
      ...
      assert.ok(
        !result.stderr.includes('ghost-agent'),
        'Agent ref in second block should be stripped'
      );
      cleanupTestDir(testDir);
      cleanupTestDir(agentsDir);
      cleanupTestDir(skillsDir);
  ...
  if (
    test('rejects agent when required field key has colon at position 0 (no key name)', () => {
      const testDir = createTestDir();
      fs.writeFileSync(
        path.join(testDir, 'bad-colon.md'),
      ...
      assert.strictEqual(
        result.code,
        1,
        'Should fail — model field is missing (colon at idx 0 skipped)'
      );
      assert.ok(result.stderr.includes('model'), 'Should report missing model field');
      cleanupTestDir(testDir);
    })
    ...
    test('rejects hook entry where command is a plain object', () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          hooks: {
            PreToolUse: [
              { matcher: 'test', hooks: [{ type: 'command', command: { run: 'echo hi' } }] },
            ],
          },
        })
      );
  ...
  if (
    test('rejects object-format matcher entry missing matcher field', () => {
      ...
      assert.ok(
        result.stderr.includes("missing 'matcher' field"),
        'Should report missing matcher field'
      );
      cleanupTestDir(testDir);
    })
  )
  ...
  if (
    test('passes on skills directory with only files, no subdirectories (Validated 0)', () => {
      ...
      assert.ok(
        result.stdout.includes('Validated 0'),
        'Should report 0 validated skill directories'
      );
      cleanupTestDir(testDir);
    })
  )
  ...
  if (
    test('skips command references on "would create:" lines', () => {
      ...
      assert.ok(
        !result.stderr.includes('phantom-cmd'),
        'Should not flag ref on "would create:" line'
      );
      cleanupTestDir(testDir);
      cleanupTestDir(agentsDir);
      cleanupTestDir(skillsDir);
  ...
  if (
    test('validates wrapped format { hooks: { PreToolUse: [...] } }', () => {
      ...
      assert.ok(
        result.stdout.includes('Validated 2'),
  ...
  if (
    test('output includes (N warnings) suffix when skill references produce warnings', () => {
      ...
      assert.ok(
        result.stdout.includes('(2 warnings)'),
  ...
  if (
    test('validates hooks in legacy array format (hooks is an array, not object)', () => {
      ...
      const result = runValidatorWithDir(
        'validate-hooks',
        'HOOKS_FILE',
        path.join(testDir, 'hooks.json')
      );
      assert.strictEqual(result.code, 0, 'Should pass on valid legacy array format');
      assert.ok(
        result.stdout.includes('Validated 1 hook'),
  ...
  if (
    test('accepts Notification and SubagentStop as valid event types', () => {
      ...
      assert.ok(
        result.stdout.includes('Validated 2 hook'),
  ...
  if (
    test('rejects skill directory with empty SKILL.md file', () => {
      ...
      assert.ok(
        result.stderr.includes('Empty file'),
