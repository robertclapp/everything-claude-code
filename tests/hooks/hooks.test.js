...
async function runTests() {
  ...
  if (
    await asyncTest('outputs session info to stderr', async () => {
      const result = await runScript(path.join(scriptsDir, 'session-start.js'));
      assert.ok(
        result.stderr.includes('[SessionStart]') || result.stderr.includes('Package manager'),
  ...
  if (
    await asyncTest('handles invalid JSON stdin gracefully', async () => {
      const result = await runScript(
        path.join(scriptsDir, 'check-console-log.js'),
        'not valid json'
      );
  ...
  if (
    await asyncTest('ignores console.warn and console.error (only flags console.log)', async () => {
      ...
      assert.ok(
        ...
        'Should NOT warn about console.warn/error/debug/info'
      );
      cleanupTestDir(testDir);
    })
  )
  ...
  if (
    await asyncTest('extracts tool names and file paths from transcript', async () => {
      ...
      assert.ok(
        result.stderr.includes('Created session file') ||
          result.stderr.includes('Updated session file'),
    ...
    await asyncTest('omits Tools Used and Files Modified sections when empty', async () => {
      ...
      if (fs.existsSync(claudeDir)) {
        ...
        if (files.length > 0) {
          ...
          assert.ok(
            !content.includes('### Files Modified'),
            'Should NOT have Files Modified when empty'
          );
          assert.ok(!content.includes('### Tools Used'), 'Should NOT have Tools Used when empty');
          assert.ok(
            content.includes('Total user messages: 2'),
  ...
  if (
    await asyncTest('slices files modified to first 30', async () => {
      ...
      for (let i = 1; i <= 35; i++) {
        lines.push(
          `{"type":"tool_use","tool_name":"Edit","tool_input":{"file_path":"/src/file${i}.ts"}}`
        );
      }
      fs.writeFileSync(transcriptPath, lines.join('\n'));
      ...
      if (fs.existsSync(claudeDir)) {
        ...
        if (files.length > 0) {
          ...
          assert.ok(
            !content.includes('/src/file31.ts'),
            'Should not include file31 (sliced to 30)'
          );
          assert.ok(
            !content.includes('/src/file35.ts'),
            'Should not include file35 (sliced to 30)'
          );
        }
      }
      cleanupTestDir(testDir);
    ...
    await asyncTest('parses Claude Code JSONL format (entry.message.content)', async () => {
      ...
      if (fs.existsSync(claudeDir)) {
        ...
        if (files.length > 0) {
          const content = fs.readFileSync(path.join(claudeDir, files[0]), 'utf8');
          assert.ok(
            content.includes('Fix the build error'),
            'Should extract string content from message'
          );
          assert.ok(
            content.includes('Also update tests'),
            'Should extract array content from message'
          );
        }
      }
      cleanupTestDir(testDir);
  ...
  if (
    await asyncTest('skips when transcript file does not exist', async () => {
      const stdinJson = JSON.stringify({
        transcript_path: '/tmp/nonexistent-transcript-12345.jsonl',
      });
      const result = await runScript(path.join(scriptsDir, 'evaluate-session.js'), stdinJson);
  ...
  if (
    await asyncTest('suggests compact at exact threshold', async () => {
      ...
      try {
        ...
        assert.strictEqual(result.code, 0);
        assert.ok(
          result.stderr.includes('5 tool calls reached'),
          'Should suggest compact at threshold'
        );
      } finally {
        try {
  ...
  if (
    await asyncTest('suggests at periodic intervals after threshold', async () => {
      ...
      try {
        ...
        assert.strictEqual(result.code, 0);
        assert.ok(
          result.stderr.includes('30 tool calls'),
          'Should suggest at threshold + 25n intervals'
        );
      } finally {
        try {
          fs.unlinkSync(counterFile);
    ...
    await asyncTest('does not suggest below threshold', async () => {
      ...
      try {
        ...
        assert.ok(
          ...
          'Should not suggest below threshold'
        );
  ...
  if (
    await asyncTest('handles COMPACT_THRESHOLD of zero (falls back to 50)', async () => {
      ...
      try {
        ...
        assert.ok(
          result.stderr.includes('50 tool calls reached'),
          'Zero threshold should fall back to 50'
        );
      } finally {
        try {
          fs.unlinkSync(counterFile);
  ...
  if (
    await asyncTest('preserves data with embedded newlines exactly', async () => {
      ...
      assert.strictEqual(
        result.stdout,
        stdinData,
        'Should preserve embedded newlines without adding extra'
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          !codeOnly.includes('shell:'),
          'post-edit-format.js should not pass shell option in code'
        );
        assert.ok(
          formatSource.includes('npx.cmd'),
          'Should use npx.cmd for Windows cross-platform safety'
        );
      }
    )
  )
  ...
  if (
    await asyncTest(
      'source code does not pass shell option to execFileSync (security)',
      async () => {
        const typecheckSource = fs.readFileSync(
          path.join(scriptsDir, 'post-edit-typecheck.js'),
          'utf8'
        );
        // Strip comments to avoid matching "shell: true" in comment text
        ...
        assert.ok(
          !codeOnly.includes('shell:'),
          'post-edit-typecheck.js should not pass shell option in code'
        );
        assert.ok(
          typecheckSource.includes('npx.cmd'),
          'Should use npx.cmd for Windows cross-platform safety'
        );
      }
    )
  ...
  function createEvalWrapper(testDir, configPath) {
    ...
    src = src.replace(
      /const configFile = path\.join\(\s*scriptDir[\s\S]*?config\.json'\s*\);/,
  ...
  if (
    await asyncTest('handles invalid config JSON gracefully (uses defaults)', async () => {
      ...
      assert.ok(
        ...
        'Should use defaults when config is invalid JSON'
      );
      cleanupTestDir(testDir);
    })
  )
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          !updated.includes('[Session context goes here]'),
          'Should replace blank template'
        );
  ...
  if (
    await asyncTest(
      'preserves existing session content when no blank template marker',
      async () => {
        const testDir = createTestDir();
        const sessionsDir = path.join(testDir, '.claude', 'sessions');
        fs.mkdirSync(sessionsDir, { recursive: true });
        ...
        assert.ok(
          updated.includes('Previous task from earlier'),
        ...
        if (fs.existsSync(claudeDir)) {
          ...
          if (files.length > 0) {
            ...
            assert.ok(
              ...
              'Should use blank template when no user messages'
            );
          }
        }
        cleanupTestDir(testDir);
      ...
      if (fs.existsSync(claudeDir)) {
        ...
        if (files.length > 0) {
          ...
          assert.ok(
            ...
            'Should extract file from nested tool_use block'
          );
    ...
    await asyncTest(
      ...
      async () => {
        ...
        try {
          ...
          assert.strictEqual(result.code, 0);
          assert.ok(
            result.stderr.includes('38 tool calls'),
            'Should suggest at threshold(13) + 25 = 38'
          );
        } finally {
          try {
            fs.unlinkSync(counterFile);
      ...
      async () => {
        ...
        try {
          ...
          assert.ok(
            ...
            'Should NOT suggest at count=50 with threshold=13'
          );
        } finally {
          try {
            fs.unlinkSync(counterFile);
  ...
  if (
    await asyncTest('passes through for non-TS extensions without running tsc', async () => {
      ...
      assert.strictEqual(result.code, 0);
      assert.ok(
        result.stdout.includes('file.js'),
        'Should pass through for .js file without running tsc'
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest('exits 0 with empty sessions directory (no recent sessions)', async () => {
      ...
      try {
        ...
        assert.ok(
          !result.stdout.includes('Previous session summary'),
          'Should not inject when no sessions'
        );
      } finally {
        fs.rmSync(isoHome, { recursive: true, force: true });
      }
  ...
  if (
    await asyncTest('does not inject blank template session into context', async () => {
      ...
      try {
        ...
        assert.ok(
          !result.stdout.includes('Previous session summary'),
          'Should skip blank template sessions'
        );
      } finally {
        fs.rmSync(isoHome, { recursive: true, force: true });
      }
  ...
  if (
    await asyncTest('stdout is exact byte match of stdin (no trailing newline)', async () => {
      ...
      assert.strictEqual(
        result.stdout,
        stdinData,
        'stdout should exactly match stdin (no extra newline)'
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest('passes through when file does not exist (readFile returns null)', async () => {
      ...
      assert.strictEqual(
        result.stdout,
        stdinJson,
        'Should pass through exactly when file not found'
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest('source calls process.exit(0) after writing output', async () => {
      const formatSource = fs.readFileSync(path.join(scriptsDir, 'post-edit-format.js'), 'utf8');
      assert.ok(
        formatSource.includes('process.exit(0)'),
        'Should call process.exit(0) for clean termination'
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest(
      ...
      async () => {
        const formatSource = fs.readFileSync(path.join(scriptsDir, 'post-edit-format.js'), 'utf8');
        assert.ok(
          formatSource.includes('process.stdout.write(data)'),
          'Should use process.stdout.write to avoid trailing newline'
        );
        ...
        assert.strictEqual(
          passThrough.length,
          0,
          'Should not use console.log(data) for pass-through'
        );
      }
    )
  )
  ...
  if (
    await asyncTest('source calls process.exit(0) after writing output', async () => {
      ...
      assert.ok(
        tcSource.includes('process.exit(0)'),
        'Should call process.exit(0) for clean termination'
      );
    })
  )
  ...
  if (
    await asyncTest(
      ...
      async () => {
        const tcSource = fs.readFileSync(path.join(scriptsDir, 'post-edit-typecheck.js'), 'utf8');
        assert.ok(
          tcSource.includes('process.stdout.write(data)'),
          'Should use process.stdout.write'
        );
  ...
  if (
    await asyncTest('exact stdout pass-through without trailing newline (typecheck)', async () => {
      ...
      assert.strictEqual(
        result.stdout,
        stdinJson,
        'stdout should exactly match stdin (no trailing newline)'
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest('does NOT match .mts or .mjs extensions', async () => {
      ...
      assert.ok(
        !result.stderr.includes('console.log'),
        'Should NOT scan .mts files for console.log'
      );
    })
  )
    passed++;
  ...
  if (
    await asyncTest('detects console.log in commented-out code', async () => {
      ...
      assert.ok(
        result.stderr.includes('console.log'),
        'Should detect console.log even in comments'
      );
      cleanupTestDir(testDir);
    })
  )
  ...
  if (
    await asyncTest('source calls process.exit(0) after writing output', async () => {
      ...
      assert.ok(
        exitCalls.length >= 2,
        `Should have at least 2 process.exit(0) calls, found ${exitCalls.length}`
      );
    })
  )
    passed++;
    ...
    await asyncTest('reports available session aliases on startup', async () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          ...
          aliases: {
            'my-feature': {
              sessionPath: '/sessions/feat',
              createdAt: new Date().toISOString(),
              updatedAt: new Date().toISOString(),
              title: null,
            },
            'bug-fix': {
              sessionPath: '/sessions/fix',
              createdAt: new Date().toISOString(),
              updatedAt: new Date().toISOString(),
              title: null,
            },
  ...
  if (
    await asyncTest('excludes session files older than 7 days', async () => {
      ...
      try {
        ...
        assert.strictEqual(result.code, 0);
        assert.ok(
          result.stderr.includes('1 recent session'),
          `Should find 1 recent session (6.9-day included, 8-day excluded), stderr: ${result.stderr}`
        );
        assert.ok(
          result.stdout.includes('RECENT CONTENT HERE'),
          'Should inject the 6.9-day-old session content'
        );
        assert.ok(
          !result.stdout.includes('OLD CONTENT SHOULD NOT APPEAR'),
  ...
  if (
    await asyncTest('handles stdin exceeding MAX_STDIN (1MB) gracefully', async () => {
      ...
      fs.writeFileSync(
        transcriptPath,
        JSON.stringify({ type: 'user', content: 'Overflow test' }) + '\n'
      );
  ...
  if (
    await asyncTest('does not inject content when session file is unreadable', async () => {
      ...
      try {
        ...
        assert.ok(
          !result.stdout.includes('Sensitive session content'),
  ...
  if (
    await asyncTest('truncates stdin at 1MB limit and still passes through data', async () => {
      ...
      assert.ok(
        result.stdout.length < payload.length,
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        fs.writeFileSync(
          sessionFile,
          '# Session file without timestamp marker\nSome existing content\n'
        );
        ...
        assert.ok(
          result.stderr.includes('Failed to update') || result.stderr.includes('[SessionEnd]'),
  ...
  if (
    await asyncTest('extracts user messages from role-only format (no type field)', async () => {
      ...
      try {
        ...
        assert.ok(
          content.includes('Deploy the production build') || content.includes('deploy'),
    ...
    await asyncTest(
      ...
      async () => {
        ...
        try {
          ...
          assert.ok(
            content.includes('/src/alt-format.ts'),
            'Should list edited file via entry.input fallback'
          );
          assert.ok(
            content.includes('/src/written.ts'),
            'Should list written file via entry.input fallback'
          );
        } finally {
          fs.rmSync(isoHome, { recursive: true, force: true });
        }
    ...
    await asyncTest(
      ...
      async () => {
        ...
        try {
          ...
          assert.strictEqual(result.code, 0, 'Should exit 0');
          assert.ok(
            result.stderr.includes('No package manager preference'),
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          result.stderr.includes('[SessionStart] Error:'),
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          result.stderr.includes('[PreCompact] Error:'),
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          result.stderr.includes('[SessionEnd] Error:'),
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        assert.ok(
          result.stderr.includes('[ContinuousLearning] Error:'),
  ...
  if (
    await asyncTest('suggest-compact exits 0 with error when TMPDIR is non-directory', async () => {
      ...
      assert.ok(
        result.stderr.includes('[StrategicCompact] Error:'),
    ...
    await asyncTest(
      ...
      async () => {
        ...
        try {
          ...
          assert.ok(
            content.includes('Refactor the auth module') || content.includes('auth'),
  ...
  if (
    await asyncTest('COMPACT_THRESHOLD exceeding 10000 falls back to default 50', async () => {
      ...
      assert.ok(
        compactSource.includes('<= 10000'),
        'Source should have <= 10000 upper bound check'
      );
      assert.ok(
        compactSource.includes(': 50'),
    ...
    await asyncTest(
      ...
      async () => {
        ...
        try {
          ...
          assert.ok(
            content.includes('Real user message'),
      ...
      async () => {
        ...
        try {
          ...
          assert.ok(
            ...
            'Should collect modified file path from tool_input'
          );
        } finally {
          fs.rmSync(isoHome, { recursive: true, force: true });
        }
...
`;
        ...
        try {
          ...
          assert.ok(
            content.includes('Some random content'),
    ...
    await asyncTest(
      ...
      async () => {
        ...
        if (result.stderr.includes('TypeScript errors in')) {
          assert.ok(
            result.stderr.includes('broken.ts'),
  ...
  if (
    await asyncTest(
      ...
      async () => {
        ...
        const script =
          'const u=require("./scripts/lib/utils");u.readStdinJson({timeoutMs:100}).then(d=>{process.stdout.write(JSON.stringify(d));process.exit(0)})';
        return new Promise((resolve, reject) => {
          ...
          child.on('close', (code) => {
            ...
            try {
              ...
              assert.deepStrictEqual(
                parsed,
                {},
                'Should resolve with {} when no data received before timeout'
              );
              resolve();
            } catch (err) {
              reject(err);
          ...
          child.on('close', (code) => {
            ...
            try {
              ...
              assert.deepStrictEqual(
                parsed,
                {},
                'Should resolve with {} when partial JSON cannot be parsed'
              );
              resolve();
            } catch (err) {
              reject(err);
