...
function runTests() {
  ...
  if (
    test('suggests compact at threshold (COMPACT_THRESHOLD=3)', () => {
      ...
      assert.ok(
        result.stderr.includes('3 tool calls reached') ||
          result.stderr.includes('consider /compact'),
  ...
  if (
    test('uses "default" session ID when CLAUDE_SESSION_ID is empty', () => {
      ...
      try {
        ...
        assert.ok(
          fs.existsSync(defaultCounterFile),
          'Counter file should use "default" session ID'
        );
